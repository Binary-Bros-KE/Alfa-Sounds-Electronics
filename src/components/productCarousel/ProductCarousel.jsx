      ...
      >
        {products.map((product) => (
          <SwiperSlide key={product.productID}>
            <div className="bg-white rounded-lg shadow-md overflow-hidden border border-green-400">
              <div className="p-4">
                <div className="text-xs text-gray-600 mb-2 font-bold">{product.categoryUSer}</div>
                ...
                <a href={`/products/${product.category}/${product.name}`}>
                  <h3 className="font-semibold text-md mb-2 line-clamp-2 capitalize hover:text-green-600 transition-all">{product.title}</h3>
                </a>
                <div className="flex items-center justify-between">
                  <div className="text-sm text-green-600 font-semibold">{product.subtitle}</div>
